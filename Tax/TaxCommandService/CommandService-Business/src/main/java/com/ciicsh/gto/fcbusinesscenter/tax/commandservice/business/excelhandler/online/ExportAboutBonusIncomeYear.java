 ...
 */
@Service
public class ExportAboutBonusIncomeYear extends BaseService {
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutBonusIncomeYear.getBonusIncomeYearWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
    ...
    public void handleBonusIncomeYearWB(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        ...
        for (TaskSubDeclareDetailPO po : taskSubDeclareDetailPOS) {
            ...
            //允许列支的捐赠比例-M列
            //准予扣除的捐赠额-N列
            //减免税额-O列tax_deduction
            HSSFCell cellO = row.getCell(14);
            if (null == cellO) {
                cellO = row.createCell(14);
            }
            cellO.setCellValue(po.getTaxDeduction() == null ? "" : po.getTaxDeduction().toString());
            //已扣缴税额-P列
            //备注-Q列
            sheetRowIndex++;
        }
