 ...
 */
@Service
public class ExportAboutRestrictiveStock extends BaseService {
    /**
     * 导出个人股票期权行权收入-限制性股票
     *
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutRestrictiveStock.getRestrictiveStockWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
    ...
    public void handleRestrictiveStockWB(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        ...
        for (EmployeeInfoBatchPO employeeInfoBatchPO : employeeInfoBatchPOS) {
            ...
            cellP.setCellValue(po.getNumberOfMonths() == null ? "" :po.getNumberOfMonths() + "个月");
            //免税所得-Q列
            //通讯费-R列
            //商业健康保险费-S列
            //税延养老保险费-T列
            ...
            //本年累计准予扣除的捐赠-V列
            //本年累计减免税额-W列
            //本年累计已纳税额-X列exercise_tax_amount
            HSSFCell cellX = row.getCell(23);
            if (null == cellX) {
                cellX = row.createCell(23);
            }
            cellX.setCellValue(po.getExerciseTaxAmount() == null ? "" : po.getExerciseTaxAmount().toString());
            //备注-Y列
            sheetRowIndex++;
        }
    }
