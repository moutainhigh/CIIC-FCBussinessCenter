 ...
 */
@Service
public class ExportAboutForeignNormalSalarySz extends BaseService{
    /**
     * 导出外籍人员正常工资薪金
     *
     ...
     * @return
     */
    public HSSFWorkbook getForeignNormalSalaryWB(List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList, String fileName, String type) {
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutForeignNormalSalarySz.getForeignNormalSalaryWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
    ...
    public void handleForeignNormalSalaryWB(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        ...
        for (TaskSubDeclareDetailPO po : taskSubDeclareDetailPOS) {
            ...
            cellT.setCellValue(po.getHousingSubsidy() == null ? "" : po.getHousingSubsidy().toString());
            //伙食补贴-U列meal_allowance
            HSSFCell cellU = row.getCell(20);
            if (null == cellU) {
                cellU = row.createCell(20);
            }
            cellU.setCellValue(po.getMealAllowance() == null ? "" : po.getMealAllowance().toString());
            //洗衣费-V列laundry_fee
            HSSFCell cellV = row.getCell(21);
            if (null == cellV) {
                cellV = row.createCell(21);
            }
            cellV.setCellValue(po.getLaundryFee() == null ? "" : po.getLaundryFee().toString());
            //搬迁费-W列removing_indemnity_fee
            HSSFCell cellW = row.getCell(22);
            if (null == cellW) {
                cellW = row.createCell(22);
            }
            cellW.setCellValue(po.getRemovingIndemnityFee() == null ? "" : po.getRemovingIndemnityFee().toString());
            //出差补贴-X列missionallowance
            HSSFCell cellX = row.getCell(23);
            if (null == cellX) {
                cellX = row.createCell(23);
            }
            cellX.setCellValue(po.getMissionallowance() == null ? "" : po.getMissionallowance().toString());
            //探亲费-Y列visiting_relatives_fee
            HSSFCell cellY = row.getCell(24);
            if (null == cellY) {
                cellY = row.createCell(24);
            }
            cellY.setCellValue(po.getVisitingRelativesFee() == null ? "" : po.getVisitingRelativesFee().toString());
            //语言培训费-Z列language_training_fee
            HSSFCell cellZ = row.getCell(25);
            if (null == cellZ) {
                cellZ = row.createCell(25);
            }
            cellZ.setCellValue(po.getLanguageTrainingFee() == null ? "" : po.getLanguageTrainingFee().toString());
            //子女教育经费-AA列education_funds
            HSSFCell cellAA = row.getCell(26);
            if (null == cellAA) {
                cellAA = row.createCell(26);
            }
            cellAA.setCellValue(po.getEducationFunds() == null ? "" : po.getEducationFunds().toString());
            //免税所得>其他费用-AB列
            //实际捐赠额-AC列
            //允许列支的捐赠比例-AD列
            //准予扣除的捐赠额-AE列donation
            HSSFCell cellAE = row.getCell(30);
            if (null == cellAE) {
                cellAE = row.createCell(30);
            }
            cellAE.setCellValue(po.getDonation() == null ? "" : po.getDonation().toString());
            //减免税额-AF列tax_deduction
