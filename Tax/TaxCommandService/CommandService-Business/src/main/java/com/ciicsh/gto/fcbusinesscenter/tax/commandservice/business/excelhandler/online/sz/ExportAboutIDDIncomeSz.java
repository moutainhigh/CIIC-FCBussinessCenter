 ...
 */
@Service
public class ExportAboutIDDIncomeSz extends BaseService {
    /**
     * 导出利息、股息、红利所得
     *
     ...
     * @return
     */
    public HSSFWorkbook getIDDIncomeWB(List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList, String fileName, String type) {
        ...
        try {
            ...
            //根据不同的业务需要处理wb
            this.handleIDDIncomeWB(wb, taskSubDeclareDetailPOList, employeeInfoBatchPOList);
        } catch (Exception e) {
            if (wb != null) {
                try {
                    wb.close();
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutIDDIncomeSz.getIDDIncomeWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
     ...
     * @param employeeInfoBatchPOList
     */
    public void handleIDDIncomeWB(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        //筛选出导出利息、股息、红利所得详细信息
        List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOS = taskSubDeclareDetailPOList.stream().filter(item -> IncomeSubject.IDDINCOME.getCode().equals(item.getIncomeSubject())).collect(Collectors.toList());
        // 读取了模板内所有sheet内容
