 ...
 */
@Service
public class ExportAboutLaborRescission extends BaseService {
    /**
     * 导出解除劳动合同一次性补偿金
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutLaborRescission.getLaborRescissionWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
    ...
    public void laborRescissionWBHandle(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        //筛选出解除劳动合同一次性详细信息
        List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOS = taskSubDeclareDetailPOList.stream().filter(item -> IncomeSubject.LABORCONTRACT.getCode().equals(item.getIncomeSubject())).collect(Collectors.toList());
        // 读取了模板内所有sheet内容
