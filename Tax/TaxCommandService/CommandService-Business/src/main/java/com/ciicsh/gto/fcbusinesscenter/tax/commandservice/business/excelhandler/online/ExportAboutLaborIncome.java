 ...
 */
@Service
public class ExportAboutLaborIncome extends BaseService {
    /**
     * 导出劳务报酬所得
     *
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutLaborIncome.getLaborIncomeWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
    ...
    public void handleLaborIncomeWB(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        //筛选出导出劳务报酬所得详细信息
        List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOS = taskSubDeclareDetailPOList.stream().filter(item -> IncomeSubject.LABORINCOME.getCode().equals(item.getIncomeSubject())).collect(Collectors.toList());
        // 读取了模板内所有sheet内容
