...
/**
 * 个人股票期权行权收入-股票增值权（深圳金三）
 * @author yuantongqing on 2018-06-01
 */
@Service
public class ExportAboutStockAppreciationSz extends BaseService{
    /**
     * 导出个人股票期权行权收入-股票增值权
     *
        ...
        try {
            ...
            if (wb != null) {
                try {
                    ...
                    //日志工具类返回
                    LogTaskFactory.getLogger().error(ee, "ExportAboutStockAppreciationSz.getStockAppreciationWB", EnumUtil.getMessage(EnumUtil.SOURCE_TYPE, "06"), LogType.OTHER, tags);
                }
            }
            throw e;
        }
    ...
    public void handleStockAppreciationWB(HSSFWorkbook wb, List<TaskSubDeclareDetailPO> taskSubDeclareDetailPOList, List<EmployeeInfoBatchPO> employeeInfoBatchPOList) {
        ...
        for (EmployeeInfoBatchPO employeeInfoBatchPO : employeeInfoBatchPOList) {
            ...
            cellL.setCellValue(po.getNumberOfMonths() == null ? "" : po.getNumberOfMonths().toString() +"个月");
            //免税所得-M列
            //通讯费-N列
            //商业健康保险费-O列
            //其他扣除-P列
            //本年累计准予扣除的捐赠额-Q列
            //本年累计减免税额-R列
            //本年累计已纳税额-S列exercise_tax_amount
            HSSFCell cellS = row.getCell(18);
            if (null == cellS) {
                cellS = row.createCell(18);
            }
            cellS.setCellValue(po.getExerciseTaxAmount() == null ? "" : po.getExerciseTaxAmount().toString());
            //备注-T
            sheetRowIndex++;
        }
    }
